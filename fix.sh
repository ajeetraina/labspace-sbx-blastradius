...
**First, export your API key in your host terminal** (don't paste
it into this page):
...
Then store it as a global sbx secret. The command below reads
`$OPENAI_API_KEY` from your shell — your key never leaves the
terminal:
...
**First, export your API key in your host terminal:**
...
```bash no-run-button
export GOOGLE_API_KEY=AIza...    # your real key
```
...
> **Why not type the key into this page?** Labspace renders anything
> you enter into an input field inline in the command below it — so
> the raw key becomes visible in the lab UI and in screenshots.
> Setting the env var in your own shell keeps the key off the page
> entirely.
