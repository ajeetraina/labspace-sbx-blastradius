...
# ---- Write docs/go.html redirect helper -------------------------------------
# This file lives outside the labspace markdown renderer (served by GitHub
# Pages from /docs on the main branch) so its JS runs normally. The form
# in 02-governance.md submits here with ?org=X&page=Y and we redirect.
HELPER_FILE="docs/go.html"
mkdir -p docs
info "Writing $HELPER_FILE (redirect helper for the governance form)"
cat > "$HELPER_FILE" <<'HELPER_EOF'
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <title>Opening Docker Admin Console…</title>
  ...
  <style>
    ...
    .card {
      max-width: 520px; padding: 32px; border: 1px solid #d0d7de;
      border-radius: 12px; background: #ffffff; text-align: center;
    }
    h1 { font-size: 18px; margin: 0 0 12px 0; }
    p  { font-size: 14px; color: #57606a; margin: 8px 0; line-height: 1.5; }
...
<body>
  <div class="card">
    <h1>Opening Docker Admin Console…</h1>
    <p id="msg">Redirecting you to the right page.</p>
    <p>If nothing happens, <a id="fallback" href="https://app.docker.com/admin">click here</a>.</p>
  </div>
  <script>
    // Redirect helper for the Labspace governance chapter quick-jump form.
    // Reads ?org=<slug>&page=<manage|network-access|filesystem-access>
    // and redirects to the matching Admin Console page.
    (function () {
      var params = new URLSearchParams(window.location.search);
      var org = (params.get('org') || '').trim().toLowerCase();
      var page = (params.get('page') || '').trim().toLowerCase();
      var msg = document.getElementById('msg');
      var fallback = document.getElementById('fallback');

      var ALLOWED_PAGES = ['manage', 'network-access', 'filesystem-access'];
      var SLUG_RE = /^[a-z0-9][a-z0-9-]{1,38}[a-z0-9]$/;

      if (!SLUG_RE.test(org)) {
        msg.className = 'err';
        msg.textContent = 'Missing or invalid org slug. Go back and type your Docker Hub org name.';
        return;
      }
      if (ALLOWED_PAGES.indexOf(page) === -1) {
        msg.className = 'err';
        msg.textContent = 'Unknown page. Pick one of the buttons in the chapter.';
        return;
      }

      var target = 'https://app.docker.com/accounts/' +
                   encodeURIComponent(org) +
                   '/admin/ai-governance/' + page;
      fallback.href = target;
      msg.textContent = 'Opening ' + target;
...
# ---- One-time GitHub Pages enablement hint ----------------------------------
# Detect whether Pages is already serving this file. If not, print a clear
# one-time setup instruction so the workshop runs smoothly.
PAGES_URL="https://${OWNER}.github.io/${REPO}/go.html"
if command -v curl >/dev/null 2>&1; then
  if ! curl -fsSL -o /dev/null --max-time 5 "$PAGES_URL" 2>/dev/null; then
    warn "GitHub Pages doesn't appear to be serving $PAGES_URL yet."
    warn "Enable it once with:"
    warn "  https://github.com/${OWNER}/${REPO}/settings/pages"
    warn "  Source: Deploy from a branch  →  Branch: ${DEFAULT_BRANCH} / /docs  →  Save"
    warn "Until that's set, the quick-jump form in the chapter will 404."
  else
    info "GitHub Pages already serving $PAGES_URL ✓"
  fi
fi
...
# ---- Commit ------------------------------------------------------------------
git add "$CHAPTER_FILE" "$YAML_FILE" "$HELPER_FILE"
